...
{	
	...
	{
		fread(&InHeader, sizeof(header_t), 1, InDataFile);

		/* Checking compatibility */
		if(InHeader.MajProgVer != MAJOR_PROG_VERSION)
		{
		...
		Rounds = InHeader.NumRounds;
		Nonce = InHeader.Nonce;
		InFileSizeByte = InHeader.DataSize;
	...
	{
		#error create header and out filename
		//MajProgVer
		//NumRounds
		//Nonce
		//DataSize
...
static uint8_t input_is_encrypted(char *InputFilename)
{
	...
	if(fread(&Header, sizeof(header_t), 1, InputFile) == sizeof(header_t))
	{
